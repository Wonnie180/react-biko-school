...
export const Home = () => {
  ...
  const { themes, themeId } = useContext(ThemeContext);

  console.log(themes, themeId);

  useEffect(() => {
    console.log("Cambiado el tema");
  }, [themeId]);
...
const Header = () => {
  const { themesLabel, themeId, modifyTheme } = useContext(ThemeContext);

  const themeLabel = themesLabel[themeId];

  return (
    <header>
      <div className="checkboxContainer">
        <span className="checkboxLabel">El tema actual es: {themeLabel}</span>
        <input
          className="checkbox"
          type="checkbox"
          onChange={(e) => {
    ...
    <section>
      ...
      <div className="inputContainer">
        <Select
          ...
          value={firstCharacterSelect}
          onChange={onFirstCharacterSelect}
        />
        <Select
          options={selectOptions}
          value={secondCharacterSelect}
          onChange={onSecondCharacterSelect}
        />
